...
bool CMT2300A_AutoSwitchStatus(uint8_t nGoCmd)
{
    ...
    while(CMT2300A_GetTickCount()-nBegTick < 10)
    {
		...
		if ((times++) > 20000)		//subGģ��������ʱֱ�ӵ�2��󷵻ش���
			return false;
    }
...
*            CMT2300A_INT_SEL_RX_FIFO_Wuint8_t
...
* @param   bEnable(true): use a single 64-uint8_t FIFO for either Tx or Rx
*          bEnable(false): use a 32-uint8_t FIFO for Tx and another 32-uint8_t FIFO for Rx(default)
* *********************************************************/
